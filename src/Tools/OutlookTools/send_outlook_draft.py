...
async def send_outlook_draft_func_async(integration_id: str, shared_mailbox_email: str = None,
                                         draft_id: str = None) -> str:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        None,
        lambda: send_outlook_draft_func(integration_id, shared_mailbox_email, draft_id)
    )


send_outlook_draft_tool = AgentTool(params=send_outlook_draft, function=send_outlook_draft_func_async, pass_context=False)
