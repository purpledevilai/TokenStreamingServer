...
async def untrash_outlook_email_func_async(integration_id: str, shared_mailbox_email: str = None,
                                            message_id: str = None) -> str:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        None,
        lambda: untrash_outlook_email_func(integration_id, shared_mailbox_email, message_id)
    )


untrash_outlook_email_tool = AgentTool(params=untrash_outlook_email, function=untrash_outlook_email_func_async, pass_context=False)
