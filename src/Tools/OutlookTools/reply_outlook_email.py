...
class reply_outlook_email(BaseModel):
    """
    Reply to the sender of an email and send immediately. The reply is threaded in the same 
    conversation as the original message. By default the reply goes to the original sender, 
    but you can override the recipient using the 'to' parameter (useful when the original 
    sender is a system/notification address and you need to reply to the actual customer).
    ...
    to: Optional[str] = Field(
        default=None,
        description="Override the recipient email address. If not provided, the reply goes to the original sender. "
...
def reply_outlook_email_func(integration_id: str, shared_mailbox_email: str = None,
    ...
    OutlookService.reply_to_message(
...
async def reply_outlook_email_func_async(integration_id: str, shared_mailbox_email: str = None,
                                          message_id: str = None, comment: str = None, body: str = None,
                                          html: bool = False, to: str = None, cc: str = None) -> str:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        None,
        lambda: reply_outlook_email_func(integration_id, shared_mailbox_email, message_id, comment, body, html, to, cc)
    )


reply_outlook_email_tool = AgentTool(params=reply_outlook_email, function=reply_outlook_email_func_async, pass_context=False)
