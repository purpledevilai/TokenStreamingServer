...
class create_outlook_reply_all_draft(BaseModel):
    """
    Create a draft reply-all to all recipients of an email without sending it. The draft is 
    threaded in the same conversation as the original message, addressed to the original sender 
    plus all To and CC recipients. The draft can be reviewed, edited, and sent later using the 
    send_outlook_draft tool. You can override or add recipients using the 'to' and 'cc' parameters.
...
def create_outlook_reply_all_draft_func(integration_id: str, shared_mailbox_email: str = None,
                                         message_id: str = None, comment: str = None, body: str = None,
                                         html: bool = False, to: str = None, cc: str = None) -> str:
    """
    Create a draft reply-all to all recipients of a message.
    ...
    Returns:
        JSON string with reply-all draft details
    """
    ...
    result = OutlookService.create_reply_all_draft(
...
async def create_outlook_reply_all_draft_func_async(integration_id: str, shared_mailbox_email: str = None,
                                                     message_id: str = None, comment: str = None, body: str = None,
                                                     html: bool = False, to: str = None, cc: str = None) -> str:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        None,
        lambda: create_outlook_reply_all_draft_func(integration_id, shared_mailbox_email, message_id, comment, body, html, to, cc)
    )


create_outlook_reply_all_draft_tool = AgentTool(params=create_outlook_reply_all_draft, function=create_outlook_reply_all_draft_func_async, pass_context=False)
