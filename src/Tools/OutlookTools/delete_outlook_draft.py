...
async def delete_outlook_draft_func_async(integration_id: str, shared_mailbox_email: str = None,
                                           draft_id: str = None) -> str:
    loop = asyncio.get_event_loop()
    return await loop.run_in_executor(
        None,
        lambda: delete_outlook_draft_func(integration_id, shared_mailbox_email, draft_id)
    )
