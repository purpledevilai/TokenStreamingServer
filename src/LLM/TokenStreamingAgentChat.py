...
class TokenStreamingAgentChat:
    ...
    async def invoke(self):
        ...
        async def _generator():
            ...
            # buffer for the AI message content
            ai_message = ""

            # Start the stream asynchronously so websocket heartbeats can be processed
            response_generator = self.prompt_chain.astream({"messages": self.messages})

            # Iterate through stream chunks
            async for chunk in response_generator:

                # 1.) Content - means it's a response for the human!
                if chunk.content:
            ...
            # 3.) Tool Section - We didn't return early, so we may have tools to call
            ...
            async for token in self.invoke():
                yield token
